...
def get_gpu_peaks():
    """
    Returns theoretical peak bandwidth and FLOPS.
    These are the two "rooflines" — no kernel can exceed them.

    PEAK BANDWIDTH: memory_clock_rate * bus_width / 8 * 2
      (×2 for DDR — double data rate, transfers on both clock edges)

    PEAK TFLOPS (FP32): SM_count * cores_per_SM * clock_rate * 2
      (×2 for FMA — fused multiply-add counts as 2 FLOPs)
    """
    prop = torch.cuda.get_device_properties(0)

    peak_bw_gbps = 2.0 * prop.memoryClockRate * (prop.memoryBusWidth / 8) / 1e6
    # FP32 TFLOPS: this is a rough estimate; exact depends on GPU arch
    # For NVIDIA: cores_per_SM varies. Ampere=128, Turing=64, Ada=128
    cores_per_sm_map = {8: 128, 7: 64, 6: 64, 5: 128}
    compute_cap = prop.major
    cores_per_sm = cores_per_sm_map.get(compute_cap, 128)
    peak_tflops = (prop.multiProcessorCount * cores_per_sm *
                   prop.clockRate * 2) / 1e9

    return {
        'name':             prop.name,
        'vram_gb':          prop.totalGlobalMem / 1e9,
